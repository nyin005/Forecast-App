...
open class BalloonMarker: MarkerImage
{
    ...
    {
        let model: BalloonMarkerModel = (entry.data as? BalloonMarkerModel)!
        let formatter = DateFormatter()
        formatter.dateFormat = "MMM yyyy"
        let strDate = formatter.string(from: Date(timeIntervalSince1970: entry.x / 1000))
        setLabel("\(strDate)\n\(model.keyDesc!): \(String(entry.y))%")
    }
